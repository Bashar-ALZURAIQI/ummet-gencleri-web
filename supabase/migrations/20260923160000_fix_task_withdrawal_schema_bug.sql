-- Fix the schema bug in cancel_task_enrollment where it incorrectly referenced an 'id' column.
-- The task_enrollments table uses a composite primary key (task_id, student_id).
...
BEGIN
  ...
  SELECT enrollment.completion_status
  ...
  DELETE FROM public.task_enrollments AS enrollment
  WHERE enrollment.task_id = p_task_id
    AND enrollment.student_id = v_user_id;
